    ...
    <header>
      ...
      <?php
        if(isset($_POST["addCategory"]))
        {
            ...
            $run = createProductCategory($db,$catName,$catParent);
            if($run)
            {
        ...
        {
            $catId = $_GET["remove"];
            $run = deleteProductCategory($db,$catId);
            if($run["result"])
            {
                header("Location:". $_SERVER["PHP_SELF"]);
            }
       ...
       <?php
            if($_SESSION["rank"] == "admin")
            {
                ...
                <form action="<?php echo $_SERVER["PHP_SELF"]?>" method="POST">
                 ...
                 <hr/>
                <?php
                $dbCategories = retrieveProductCategories($db);
                if($dbCategories)
                {
                    ?>
                    <table id="data-table">
                        ...
                        foreach($dbCategories as $dbCat)
                        {
                            ...
                            $catParent = retrieveProductCategoryName($db,$dbCat["catParent"]);
